...
A implementação do protocolo Raft apresentada neste relatório segue convenções modernas de outras implementações que facilitam os testes automatizados e aumentam a confiança na corretude da implementação do algoritmo.

Além disso, embora o algoritmo Raft completo especifique mecanismos para mudança dinâmica de configuração (como adição e remoção de nós) e compactação de logs através de snapshots para gerenciar o crescimento do estado físico, essas funcionalidades não foram incluídas na presente implementação. O raciocínio para essa decisão de design reside no fato de que o núcleo implementado — contendo a eleição de líder estável, a replicação básica de entradas e a recuperação de logs consistentes — é suficiente para atingir consistência sequencial na máquina de estados replicada. Em ambientes controlados ou acadêmicos, a configuração estática do cluster e a ausência de snapshots não comprometem a correção das propriedades de segurança e liveness do consenso, mantendo a integridade da ordem de execução de todas as operações por todos os nós.

Mesmo sem a capacidade de compactar logs ou mudar a configuração do cluster dinamicamente, implementar o algoritmo corretamente se mostrou um desafio, pois ambientes distribuídos são inerentemente complexos e as interações entre nós podem ser muito sutis. Nesse sentido, os testes randomizados reprodutíveis se provaram muito valiosos, pois eram capazes de testar grande parte do espaço de estados possíveis e fornecer uma seed de reprodução para os casos que falharam. Durante o desenvolvimento, muitos bugs foram encontrados através desses testes, que também facilitaram refatorações, garantindo que os bugs consertados previamente não regredissem.

No geral, essa experiência demonstrou a complexidade e dificuldade de se implementar algoritmos distribuídos corretamente, e a importância de se desenvolver a implementação pensando numa estratégia de testes completa desde o início.
